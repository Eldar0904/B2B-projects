...
## 19. Thirteenth pass (30 July 2026) - LaBSE actually verified in real
Docker, LOW_CONFIDENCE_THRESHOLD removed, and a real infra bug found +
fixed along the way (healthcheck timeout, then a 10-12 min-per-restart
re-embedding cost)

### 19.1 LaBSE confirmed working, with real numbers

After several rounds of `docker compose up --build` not actually taking
effect (see 19.2), the user got a genuinely fresh rebuild and ran
`calibrate_embedding_provider.py` for real:
`embedding_provider=sentence-transformers`, `embedding_dim=768` (LaBSE's
real output size, vs. TF-IDF's fallback 384 - confirms this run was
real, not another silent fallback). Results:

- top-1 100.0% (34/34), top-3 100.0% - vs. the tfidf-era baseline's
  97.1%/100%.
- Score separation genuinely IMPROVED, not worsened, despite the script's
  own generic caution line: true-match p10 stayed exactly 0.949 (unchanged)
  while the random-sample p90 DROPPED from 0.718 to 0.614 - non-matches got
  pushed further away from the threshold zone, widening the real gap
  (0.335 vs. tfidf's 0.231), which is what actually matters for a
  threshold to mean something.
- Known hard cases: 2 of 4 matched expectations closely (both "should stay
  low, not in catalog" cases did). One scored lower than hoped (a real
  match landing in manual review instead of auto-confirming - acceptable,
  still reaches a human with the right candidate attached). One
  ("mop with platform" test case) scored 0.185 - just under the OLD 0.20
  no-match cutoff, meaning it would have reached ZERO reviewers under the
  previous config. This became the direct, concrete justification for 19.3.

### 19.2 Real infra bugs found and fixed getting there

Two genuine Docker/infra bugs, not code bugs, cost most of a session to
diagnose - worth documenting precisely since they will recur for anyone
touching `.env` or the Dockerfile again:

1. **`env_file:` values are baked into a container at CREATION time, not
   read live from the bind-mounted file.** Editing `backend/.env` and
   running `docker compose up --build` repeatedly kept showing
   `embedding_provider=tfidf` and `ModuleNotFoundError: No module named
   'sentence_transformers'` - the running container was 6+ hours old,
   just being *restarted* (`docker start`, same container ID, CREATED
   timestamp unchanged) rather than recreated, so it never picked up
   either the new `.env` value or the new image. Diagnosed via `docker ps`
   showing `CREATED: 6 hours ago` next to `STATUS: Up 2 minutes` - that
   specific combination is the tell. Fix: `docker compose down` (actually
   removes containers) before `docker compose up --build`, or
   `docker compose up -d --force-recreate <service>` for a targeted config
   change that doesn't need a full image rebuild.
2. **The backend healthcheck's `start_period: 60s` was calibrated for the
   old TF-IDF startup cost (~11s) and never updated for LaBSE.** Once the
   image genuinely had `sentence-transformers` installed, `docker compose
   up -d` failed with "container product_matching_backend is unhealthy" -
   but the container was NOT crashed, `docker ps`/Docker Desktop showed
   ~780% sustained CPU (real transformer inference over ~21,326 products,
   measured at ~12 minutes total). Fixed in `docker-compose.yml`:
   `start_period: 20m`. A genuine crash still surfaces well within that
   window; a real backend just doing real work no longer gets killed for
   being slow.

### 19.3 LOW_CONFIDENCE_THRESHOLD removed (0.20 -> 0.0)

Direct user request after seeing 19.1's hard-case results: "reduce NO
match at maximum, give people variants to choose for all items."
`build_job_result`'s only gate between `needs_review` and `no_match` for
anything not auto-matched/LLM-confirmed is `p.top_score >=
settings.low_confidence_threshold` (`standalone_matching.py`) - set to
`0.0`, this is always true for any item with at least one real candidate
(scores are clamped to `[0, 1]` in `vector_search.py`), so an item only
still lands in `no_match` when the search index returned literally zero
candidates (an empty catalog - not a real case with any catalog loaded).
No code change needed, purely a `.env` value - but **caught and fixed a
real mistake while making it**: the first edit put the literal Cyrillic
item name ("mop with platform") directly in a `.env` comment, violating
this project's own "keep `.env` pure ASCII" rule from HANDOFF section 2 -
caught immediately by the same ASCII-check habit used throughout this
session, fixed before it could crash anyone's Windows install.

This needs a container **recreation** (`docker compose up -d
--force-recreate backend`), not a plain restart, for the same env_file
reason as 19.2's first bug - and, without 19.4 below, would have cost
another ~12 minutes for the re-embedding no-op. **Confirmed done by the
user this session** (`docker compose up -d --force-recreate backend`
succeeded).

### 19.4 New: on-disk build cache so a restart doesn't re-embed everything

Real pain surfaced directly by 19.2/19.3: with LaBSE active, EVERY backend
process start - not just ones where the catalog actually changed - paid
the full ~10-12 minute re-embedding cost, because
`_rebuild_index_if_master_catalog_exists` (`main.py`) calls
`CatalogSearchIndex.build()` unconditionally on every startup, and
`VectorIndex.build()` (`vector_search.py`) always deleted + recreated the
Qdrant collection and recomputed every embedding from scratch, regardless
of whether Qdrant already durably had the exact same data from the
previous process's run (Qdrant itself survives a backend restart fine -
only the backend's own code was throwing that away and redoing it every
time).

Fix: a small on-disk JSON manifest (`backend/.index_cache/<collection>.json`
- bind-mounted, so it survives a container recreation, not just a
restart) records a SHA-256 content fingerprint (sorted by record id, so
order-independent) of the last successfully-indexed record set. On the
next `build()` call, if the fingerprint matches AND Qdrant's collection
still verifiably exists with the same point count (never trusts the
manifest alone - a stale manifest next to a wiped/fresh Qdrant volume
must still trigger a real rebuild), the expensive part - `embed()` across
the whole corpus, plus the delete/recreate/upsert cycle - is skipped
entirely. `fit()` still always runs (cheap for both providers; required
so a later query-time `embed()` call works correctly for
`TfidfEmbeddingProvider`, which silently self-fits on whatever it's first
given if `fit()` was never called).

**Real correctness risk caught and fixed while building this, not after**:
Qdrant point ids are assigned positionally (`id=i` for `indexable[i]`).
`loader.load_master_records()` had no `ORDER BY` at all - two calls with
IDENTICAL content could still return rows in a different sequence (a real
risk under PostgreSQL, which makes no ordering guarantee without one),
which would silently associate a cache-hit's point ids with the WRONG
records the next time IDs happened to shuffle. Fixed at the source: added
`.order_by(MasterProduct.id)` - every caller benefits from deterministic
ordering now, not just this cache.

Tests: `tests/test_vector_search.py` gained an autouse fixture isolating
every test's cache directory (so test runs never touch the real
`/app/.index_cache`, and can't pollute each other), plus five new cases -
a cache hit skips `embed()` entirely, a real content change still
triggers a real rebuild, search results stay correct after a skipped
build (not just "embed() wasn't called" - proving the reused collection
is genuinely still queryable), a corrupt manifest file falls back to a
real rebuild instead of crashing, and a valid manifest next to a
wiped/missing Qdrant collection is NOT trusted on its own.

**NOT verified - no Docker access in this sandboxed session.** Whoever
restarts the backend next should see this directly: watch
`docker compose logs backend -f` after a restart with an UNCHANGED
catalog - it should reach `Application startup complete` in well under a
minute (BM25/fuzzy/overlap indexes still rebuild fresh every time - fast,
in-memory, never were the bottleneck), not another ~12-minute wait. Run:

```
docker compose exec backend pytest -q tests/test_vector_search.py
```

then restart the backend twice in a row with nothing else changed and
confirm the second restart is dramatically faster than the first. If the
catalog genuinely changes (an edit, a merge, a new upload), the next
restart (or the next real `/api/search/reindex` call, or Catalog tab
mutation) should still correctly do a full rebuild - worth a quick manual
check too, since a caching bug that never invalidates is worse than no
cache at all.

**Real verification, same session**: the user confirmed this directly.
First restart after the fix landed took the expected ~12 minutes (nothing
to cache yet); a genuinely fresh reset (`docker compose down` + `up -d`,
needed anyway to clear an unrelated stuck-duplicate-container mess) then
came up `Up 5 minutes (healthy)` well within that window, log flowing
straight from `rebuilding search index...` to `Application startup
complete` with no gap - the cache hit for real.

### 19.5 LLM auto-match: decided, dropped

NEXT_STEPS.md item 3's three-way decision was made explicitly by the
user: **drop it**, not move to a paid tier or accept the free-tier
ceiling. `ENABLE_LLM_AUTO_MATCH=false` in `.env` - `_build_auto_match_confirmer`
already logs why and returns `None` when this is off, falling open to
ordinary manual review with zero code changes needed. Left fully
implemented, not deleted (no git, no cost to keeping unused code around -
flip it back to `true` any time the quota math changes).

**Explicitly NOT touched by this decision**: `ENABLE_LLM_RERANKER_FOR_HARD_CASES`
(section 7's tie-breaker) stays on - it's a fundamentally different usage
pattern (one ambiguous top-2 call per hard case during reranking, not a
batch pass over potentially hundreds of pending items), so it doesn't hit
the same quota wall the auto-match confirmer did. Worth remembering these
are two separate flags before assuming "LLM stuff is off" covers both.

### 19.6 New: manual catalog search in the wizard (NEXT_STEPS item 6)

The other never-built wizard gap, alongside "no resume" (section 17): if
none of the 3 auto-suggested candidates fit, the only previous option was
"Не подходит" - no way to search the catalog and pick a DIFFERENT item.
The old Review tab had exactly this via `POST /api/matching/{upload_id}/manual-search`,
but that endpoint queries `index_manager.get_index()` - the catalog-
agnostic GLOBAL singleton index the main app's Upload & Review flow uses -
which is not necessarily (and often isn't) the catalog a given wizard run
is actually matching against.

New `standalone_matching.manual_search(db, catalog_version, query, top_k)`
+ `POST /api/v1/matching/manual-search` instead: scoped to the SAME
per-`catalog_version_id` cached index `run_matching_job` already reads/
writes (`get_cached_index_for_version`/`cache_index_for_version`), so a
search always returns results from the catalog the reviewer is actually
working against - rebuilding once on a cache miss (e.g. right after a
restart), same pattern as every other cache-miss path in this file. Uses
`index.search()` (raw retrieval), not `search_reranked()` - a manual
search is the reviewer explicitly overriding what the algorithm already
decided was NOT the top 3, so it should show the broadest reasonable
candidate set rather than reapply the same reranking logic.

Frontend: `frontend/public/matching/{index.html,app.js,styles.css}` - a
search box below the 3 candidate buttons on the wizard's per-item screen,
prefilled with the item's own destination name (editable) as a reasonable
starting point. Search results render with the same `formatProductCard`
markup as the auto-suggested candidates. Picking one **merges it into
`record.candidates`** (if not already present) before calling the
existing `saveChoice()` - deliberate, not incidental: every downstream
helper (`findCandidate`/`findCandidateName`/`findCandidateScore`, the
detail views, `buildSaveRows()`) already knows how to look a choice up in
`record.candidates`, so a manually-found item needs zero special-casing
anywhere else in the file once it's merged in.

Tests: `test_standalone_matching.py` gained four service-level cases -
results come from the right catalog, results NEVER leak from a different
catalog version (two disjoint catalogs seeded specifically to prove this -
the actual reason this function exists instead of reusing the old
endpoint), a cache miss rebuilds correctly, and an already-cached index is
reused rather than silently rebuilt from a possibly-stale DB read.
`test_standalone_matching_api.py` gained the HTTP-level wiring: candidates
returned, 404 for an unknown catalog version, 400 when
`catalog_version_id` is missing entirely, and an empty/whitespace query
returns `[]` rather than erroring. Both test files' autouse fixtures were
extended to isolate the on-disk index-build cache (section 18) the same
way `test_vector_search.py` already does, since this is the first time
these two files actually build a real index that would otherwise touch
the shared default cache path.

**NOT verified - no Docker/browser access this session.** Run:

```
docker compose exec backend pytest -q tests/test_standalone_matching.py tests/test_standalone_matching_api.py
```

then open the wizard, get to a manual-review item, and confirm: the
search box is prefilled with the item's name, searching returns real
catalog results, picking one advances to the next question exactly like
picking an auto-suggested candidate, and the final export/save correctly
records that pick.

### 19.7 Fixed same session: the cross-catalog Qdrant collision manual
search's tests found (NEXT_STEPS item 6a)

Writing `test_manual_search_never_returns_results_from_a_different_catalog_version`
(section 19.6) was the first time this project actually built two
different `CatalogVersion`s' indexes side by side in one test - and it
crashed, revealing a real, previously-undocumented bug: every catalog
shared ONE hardcoded Qdrant collection name
(`settings.qdrant_collection_name`, always `"products"`), and
`VectorIndex.build()` always deletes and fully recreates whatever
collection it's given. `index_manager.py`'s `_version_index_cache` holds
several `CatalogSearchIndex` objects at once specifically so the wizard's
"use existing catalog" picker can switch between them without restarting
- but building a second, different catalog's index while an older one was
still cached silently wiped the older catalog's real Qdrant data out from
under its still-"valid" cached object. Not a crash: a later search
against that stale cache would query a client that still LOOKED correct
but was now pointed at the wrong catalog's vectors, returning a
plausible-looking but wrong candidate with no error at all. Real risk,
not theoretical, given this project's own incremental catalog upsert
(April -> May, section 15) actively encourages keeping multiple catalog
versions around.

**Fix**: `CatalogSearchIndex.build()` (`index_manager.py`) gained an
optional `collection_name` parameter, defaulting to
`settings.qdrant_collection_name` - so the main app's global singleton
index (`main.py`'s startup rebuild, `POST /api/search/reindex`, the
ML/active-learning flows) is completely unaffected, unchanged behavior.
New `standalone_matching._qdrant_collection_name(upload_id) -> str`
(`f"catalog_{upload_id}"`) gives every per-catalog build a name derived
from its master Upload id - not its CatalogVersion id, because
`run_matching_job`'s fresh-upload branch builds the index BEFORE the
CatalogVersion row exists (`_create_catalog_version` needs the build's
own `stats.indexed_records`) - the upload id is the one identifier
that's always available at build time either way. All three per-catalog
build call sites (`run_matching_job`'s reuse-existing-catalog and
fresh-upload branches, `manual_search`'s cache-miss rebuild) now pass this
explicitly. Two different catalogs' builds can no longer structurally
touch the same collection - not "shouldn't", genuinely can't.

**Deliberately NOT done in this pass**: no automatic cleanup of Qdrant
collections belonging to a deleted/stale CatalogVersion - they now
accumulate one-per-catalog forever, the same "never destroy data
implicitly, only via an explicit action" convention this project already
applies to MasterProduct rows (cleaned up by dedicated scripts, not
automatically). Worth a `cleanup_orphaned_qdrant_collections.py`-style
script eventually if this becomes a real storage concern, following the
same dry-run-by-default pattern as `cleanup_stale_catalog_versions.py` -
not needed yet.

Tests: `test_qdrant_collection_name_is_distinct_per_upload` - a direct,
Qdrant-free check of the exact invariant the whole fix depends on. The
existing `test_manual_search_never_returns_results_from_a_different_catalog_version`
still needs a workaround (a second local Qdrant path for the second
build) for a SEPARATE, unrelated reason: embedded/local-mode Qdrant
exclusively locks its storage DIRECTORY (not per-collection), so two
simultaneously-live clients on one path still crash regardless of which
collection each targets - a real deployment's actual Qdrant SERVER has no
such restriction (local mode's own error message says so explicitly:
"If you require concurrent access, use Qdrant server instead"). That
test's docstring was updated to explain this precisely, rather than
implying the workaround was still needed for the bug this section fixes.

**NOT verified - no Docker access in this sandboxed session.** Run:

```
docker compose exec backend pytest -q tests/test_standalone_matching.py tests/test_standalone_matching_api.py tests/test_index_manager.py
```

(the last one to confirm the global singleton index's default behavior -
unchanged - wasn't broken by the new optional parameter). Real end-to-end
confidence needs a live check too: run the wizard against two different
catalogs in one session without restarting, then switch back to the
first one and confirm it still returns its own real candidates.

### 19.8 Attribute-comparison scoring signal: rebuilt with the suspected
fix, benchmarked, decision still pending real numbers (NEXT_STEPS item 5)

The second-biggest documented accuracy lever, previously rejected: a
version of this (`attribute_score.py`, since deleted) made real accuracy
WORSE at every weight tested (80.0% -> 65.7% as weight rose to 0.20).
Section 5's own suspicion, never actually isolated and tested until now:
`material_similarity` was a blunt binary (1.0/0.0), and real matches
disagree on material almost as often as real non-matches happen to agree
- dragging down correct matches without reliably catching wrong ones.

New `app/services/search/attribute_score.py` targets that theory
specifically:

- `AttributeProfile` - a tiny shared dataclass for the four columns both
  `MasterProduct` and `DestinationProduct` already carry
  (`dim_w_mm`/`dim_h_mm`/`dim_d_mm`/`material`).
- `dimension_similarity` - ratio similarity (`min/max`) per axis, averaged
  over only the axes present on BOTH sides; returns `None` (not 0.0) when
  no axis is comparable at all - a row with only width parsed must not be
  treated as "definitely different" from one with only height parsed.
- `material_similarity` - same binary exact-match idea as before (this
  part isn't the fix), but ALSO returns `None` when either side has no
  material parsed, instead of implicitly forcing a 0.0.
- `compute_attribute_score` - weights dimension at 0.85 and material at
  0.15 (NEXT_STEPS' own suggested fix - "weight dimension similarity much
  higher than material"), and uses the NEUTRAL midpoint 0.5 (not 0.0) for
  whichever half has no comparable data - the actual, direct fix for the
  suspected root cause. Only returns the flat 0.5 baseline when NEITHER
  dimensions nor material can be compared at all (the common case, since
  `attributes.py` only populates these when the source text actually
  mentions them) - "unknown" now honestly means unknown, never
  indistinguishable from "confirmed mismatch".

**Genuinely verified, not just written**: this module has zero external
dependencies (pure functions, no DB/Qdrant/config), so - unlike every
other Python change this session - it could actually be tested directly
in this sandbox instead of only asked to be trusted. `pip install pytest`
+ `pytest tests/test_attribute_score.py` was run for real here: **12/12
passed**, including the two tests that matter most - missing material
never drags down a real dimension match, and a dimension mismatch
outweighs a material match by design (directly exercises the 0.85/0.15
split with concrete numbers, not just documentation asserting it).

**Still NOT wired into production scoring** - deliberately.
`ScoringWeights`/`compute_final_score` are completely untouched; this
project's own rule after the first rejection is explicit: never enable an
attribute weight without a real measured benchmark showing it helps.

New `scripts/calibrate_attribute_score.py` (same safety scaffolding as
`calibrate_embedding_provider.py` - never commits, isolates Qdrant into
its own temporary collection): fetches each ground-truth item's REAL
top-20 candidate pool once (`matching.get_top_candidates`, completely
unchanged), then re-scores and re-sorts that pool at each of several
blend weights (0.00 through 0.30) purely inside the script, checking
top-1 accuracy and score separation at each one. Weight 0.00 is
mathematically identical to the current unmodified baseline (`blended =
(1-0)*final_score + 0*attribute_score = final_score`), so it should print
numbers matching `calibrate_embedding_provider.py`'s own baseline
exactly - a built-in sanity check that the harness itself is wired
correctly, not just that the signal happens to look good.

Uses the SAME validated Казниса апрель/Детсад ground truth already
confirmed sound for the LaBSE calibration (section 16/19.1) - not the
original benchmark's "auto-selected newest master upload paired with
newest destination upload" approach, which section 5 flagged as a
possible confound (no guarantee those two uploads were ever meant to
correspond to each other).

**NOT run - needs Docker access this session doesn't have.** Run:

```
docker compose exec backend python -m scripts.calibrate_attribute_score
```

and read the printed table. The decision rule is explicit in the
script's own final printout: only wire a weight into `ScoringWeights` if
BOTH top-1 accuracy improves AND the score-separation gap (`true p10 -
sample p90`) widens at that weight, compared to the weight-0.00 baseline
row. If every weight above 0.00 makes either number worse, this rewrite
failed too, for a genuinely different reason worth knowing even so - and
should stay unwired, exactly like the original.

### 19.9 Benchmark actually run (real Docker output) - wired into
production, real numbers, real caveat about what weight 0.00 means

The first real run (section 19.8's script, as first written) came back
weight 0.00 = 79.4%, not the expected 100% match against
`calibrate_embedding_provider.py`. Two false leads before the real cause,
both worth recording since they looked like real bugs and weren't quite:

1. First attempt blended the new signal with `final_score` but re-derived
   weight 0.00 by re-sorting a RAW (non-reranked) pool - printed 85.3%,
   wrong, because `get_top_candidates` returns the Phase 6 RRF-reranked
   order, not raw retrieval order.
2. Second attempt blended with `reranker_score` instead, reasoning that's
   what the pool is actually sorted by - but RRF's `reranker_score` is a
   rank-fusion value (~0.03-0.05 in a real run), not a `[0, 1]` confidence
   number like `attribute_score` - blending two incompatible scales made
   the attribute term numerically dominate at almost any weight, which is
   why accuracy fell monotonically in that run. That result said nothing
   real about the attribute idea - it was a units mismatch.

Fixed by special-casing weight 0.00 to return `candidates[0]` UNCHANGED
(so it is definitionally identical to the real, already-reranked top-1),
and blending `final_score` (genuinely `[0, 1]`, same scale as
`attribute_score`) at weight > 0.

Even after that fix, weight 0.00 still printed 79.4%, not 100% - THIS was
real, but not a bug in the script: `ENABLE_LLM_RERANKER_FOR_HARD_CASES=true`
was still on (section 7), and `CatalogSearchIndex.build()` reads that flag
at build time to decide whether to wrap the base RRF reranker in an
`LLMReranker` tie-breaker that makes live, non-deterministic Gemini calls.
Two separate process runs of the same script can legitimately disagree on
an ambiguous tie's outcome - not a defect, just non-determinism the
benchmark hadn't accounted for. Fixed by having the script force
`settings.enable_llm_reranker_for_hard_cases = False` for the duration of
its own run (restored in `finally`), making it fully self-contained and
reproducible. This also means its weight-0.00 number is NOT expected to
match `calibrate_embedding_provider.py`'s own reported baseline anymore -
that script still runs with the tie-breaker on, so the two scripts are
now deliberately measuring different things (LLM-assisted vs LLM-free).

**The clean, deterministic run** (30 July 2026, real Docker output, LaBSE
embeddings, n=34 ground truth):

```
weight   top-1 acc   true mean   true p10   sample mean   sample p90   gap
0.00       67.6%       0.914      0.656         0.375        0.661     -0.005
0.05       85.3%       0.914      0.756         0.381        0.664      0.092
0.10       85.3%       0.893      0.742         0.388        0.659      0.083
0.15       85.3%       0.872      0.729         0.394        0.653      0.075
0.20       82.4%       0.851      0.715         0.401        0.644      0.071
0.30       70.6%       0.814      0.729         0.414        0.626      0.103
```

This is a genuinely important number in its own right, independent of the
attribute-score question: **67.6% is the real accuracy of the RRF
reranker with zero LLM help.** The earlier "100% with LaBSE" figure
(section 19.1) was never a pure-embedding number - it always had the LLM
tie-breaker live, quietly doing real work resolving ambiguous ties. Since
`ENABLE_LLM_AUTO_MATCH` was already dropped for quota reasons (section
19.5) and the tie-breaker reranker has the same real-Gemini-call quota
exposure, this 67.6% is a more honest picture of what the system delivers
without relying on API availability.

Against that honest baseline, weight 0.05 is a real, structurally
different result from the first (deleted) attempt: accuracy 67.6% ->
85.3%, and score separation goes from essentially nothing (-0.005 - true
matches and random non-matches overlap at the p10/p90 level) to a real
gap (+0.092), using zero LLM calls. 0.05 is the best point on both
accuracy and separation; 0.10-0.15 hold accuracy but separation erodes;
0.30 falls apart (70.6%), likely overfitting attribute noise at n=34.

**Wired into production**, at weight 0.05, deliberately NOT as a
`ScoringWeights` field:

- `config.py`: new `attribute_score_weight: float = 0.05` (0.0 disables
  the whole rescoring step, including the extra per-candidate DB
  attribute lookups - exact pre-existing behavior).
- `matching.get_top_candidates`: when the weight is > 0, requests a wider
  already-RRF-reranked pool (`settings.rerank_pool_size`, not just
  `top_k`), blends each candidate's `final_score` with
  `attribute_score.compute_attribute_score(destination_profile,
  master_profile)` at that weight, re-sorts, then truncates to `top_k`.

**Why not a `ScoringWeights` field**, the way the first (deleted) attempt
and the initial plan for this one both assumed: `RRFReranker.rerank`
(reranking.py) computes its fusion purely from each candidate's
`keyword_score`/`fuzzy_name_score`/`embedding_score` RANKS - it never
reads `final_score` at all. Adding an attribute weight to `ScoringWeights`
would only change which candidates survive `retrieve_candidates`'s own
top-20 truncation (the pool BEFORE RRF), never the RRF-decided final
order - it would not reproduce what was actually measured here. The
post-rerank blend in `get_top_candidates` is the only place that matches
the benchmark's own methodology exactly.

**Verified for real in this sandbox** (no Docker needed - `attribute_score.py`
and this wiring only touch already-fetched ORM rows, same as before):
`tests/test_matching.py` gained two new tests, both actually run via
`pytest` here, not just written - `test_attribute_rescoring_is_a_no_op_when_nothing_has_parsed_attributes`
(the common case: no dim/material data anywhere must not change the
existing fixture's ranking, since a constant NEUTRAL offset can't reorder
anything) and `test_attribute_rescoring_can_promote_a_better_dimension_match`
(two textually-identical master rows, only one sharing the destination's
real parsed dimensions - attribute blending must promote it to rank 1, at
the real production weight, not a stub). Full suite: 24/24 in
`test_matching.py` + `test_attribute_score.py`, 188 passed / 2 skipped
across the rest of the suite (a handful of pre-existing, unrelated
collection errors are this session's known sandbox bash-mount truncation
artifact on files this change never touched - confirmed via the Read
tool, not trusted from bash, same protocol as every other case of this
this session).

**NOT yet confirmed in real Docker** - run
`docker compose exec backend pytest -q tests/test_matching.py` there too,
then a real click-through: open the wizard against an upload where some
rows actually have parsed dimensions (a "800x680x400" style pattern in
the source text - `attributes.py` is what parses these), and confirm the
top-3 shown make sense. `attribute_score_weight` in `.env` can be set to
`0.0` at any time to instantly revert to the pre-19.9 behavior with no
code change if this ever looks wrong on more real data.

---

## 20. Fourteenth pass (31 July 2026) - correction: this project HAS git,
section 19's work committed, cleanup.ps1 still not run

### 20.1 Correction to every prior "no git in this project" statement

This session found what every previous session assumed didn't exist: a
real git repo, with a real GitHub remote already configured
(`github.com/hantishka-byte/GoodsProgram`, remote name `main`), 5 commits
before this session started, most recently 30 July
("Add standalone matching widget with embedding-based similarity",
author `it-pine-group <it@pine.kz>`, co-authored by a prior Claude
session). Every "no git = no rollback safety" caution in this file,
`NEXT_STEPS.md`, and the 31 July session summary was **stale** - written
by sessions that either didn't check or checked before the repo was
initialized. `.env` was correctly gitignored the whole time and was never
committed - no secret exposure.

All of section 19's work (LaBSE wiring, attribute-score signal, Qdrant
collision fix, on-disk build cache, manual search) was sitting as
uncommitted local changes when this session started - real risk of loss,
not a hypothetical one. Committed as `4ecd52a` ("Wire up LaBSE embeddings,
attribute-comparison scoring, and fix cross-catalog Qdrant collision"):
the 18 modified tracked files plus the new files required for that code
to actually run (`attribute_score.py` - `matching.py` imports it directly
- plus its test and both calibration scripts). Verified this was
necessary, not optional: `git show HEAD:.gitignore` right before the
commit still showed the pre-edit content even though the `Read` tool
showed the new `backend/.index_cache/`/`backend/calibration_data/` lines
- the known bash-mount sync lag (section 2) affects git's view of the
working tree too, not just this session's own edits. Confirmed via `git
diff --cached` that the actually-staged content of the 22 files matched
what `Read` showed, not a stale snapshot, before trusting the commit.

**Deliberately left uncommitted** - a separate, larger backlog spanning
sections 13-18, not part of what looked at-risk this session:
`catalog.py`, `attributes.py`, `catalog_merge.py`, the `migrate_*.py`/
`cleanup_*.py` scripts, `CatalogTab.tsx`, `test_catalog_api.py`,
`test_catalog_merge.py`, `test_attributes.py`, `entrypoint.sh` (not
referenced by the current `Dockerfile`/`docker-compose.yml` - orphaned,
purpose unconfirmed). Also deliberately excluded: `backend/calibration_data/`
(two real customer files - `Детсад.xlsx`, `Казниса апрель.xlsx`, ~3MB
combined) and `backend/.index_cache/` (machine-local cache manifests) -
added both to `.gitignore` rather than pushing real customer data to
GitHub without an explicit decision to do so. The loose root-level
`SESSION_SUMMARY_*.md`/`NEXT_STEPS.md`/`Session_Report_*.md` files and
the root `*.xlsx` test-data files are also still untracked - not touched
this pass.

### 20.2 Push blocked - needs to happen from the user's own machine

This sandbox has no GitHub credentials (`git push` fails with "could not
read Username for 'https://github.com'" - no credential helper
configured here, by design). The commit above exists only in the local
repo on the user's machine (this sandbox mounts that same folder, so the
commit IS real and present there) - it has NOT reached GitHub yet. Whoever
picks this up needs to run, from a real terminal on the actual machine
(not this sandbox):
...
### 20.3 Real, non-cosmetic sandbox limitation found: git operations leave
stray lock files here

Every git write command in this session (`add`, `commit`, `status`) hit
`fatal: Unable to create '.../.git/index.lock': File exists` on its
SECOND invocation, because the first invocation's own lock file couldn't
be unlinked at the end (`Operation not permitted`) - the same delete-
permission block documented for `backend/catalog_audit.py` (section 18.2)
turns out to also block git's own internal cleanup, not just this
project's files. Worked around by `mv`-ing (not `rm`-ing) the stray lock
out of the way before each subsequent git command - rename succeeded
where unlink didn't, for reasons not fully understood (possibly a FUSE/
bind-mount quirk where rename and unlink hit different code paths).

This left several harmless `.git/index.lock.bak.*`, `.git/HEAD.lock.bak.*`,
and `.git/objects/maintenance.lock.bak.*` files sitting inside `.git/` -
git itself ignores files that aren't named exactly `index.lock`/
`HEAD.lock`, so they should not block anything, but whoever runs the
`git push` above should delete them by hand first (trivial outside this
sandbox, where normal delete permissions apply) for tidiness. This is
purely a this-sandbox artifact - git running natively on the user's own
machine has never shown this behavior in any prior session.

### 20.4 cleanup.ps1 - confirmed still not run

`backend/.venv` (318M), `frontend/node_modules` (273M), and
`backend/catalog_audit.py` are all still present, unchanged from the 31
July session summary's report - `cleanup.ps1` has not been run yet. Same
sandbox delete-permission block as section 20.3 means this still can't be
done from here; needs a direct run on the user's machine (right-click
`cleanup.ps1` -> Run with PowerShell, as already documented in
`NEXT_STEPS.md`).

### 20.5 Still blocked this session, same as every prior session: no
Docker, no browser

This sandbox has no `docker`/`docker-compose` binary and no browser access
- confirmed by checking directly rather than assuming. Every NEXT_STEPS
item that needs either (the attribute-score Docker pytest run, the
22009->27202 master-product-count sanity check, all of the wizard-resume/
manual-search/Qdrant-collision/Catalog-tab browser click-throughs) remains
exactly where the 31 July session summary left it - genuinely not done,
