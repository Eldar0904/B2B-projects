...
function renderWizardQuestion() {
  ...
  // Manual catalog search (NEXT_STEPS.md item 6) - reset for every new
  // question, prefilled with the item's own name as a reasonable starting
  // point (editable) rather than making the reviewer retype it. Results
  // from the PREVIOUS item must never linger onto this one.
  $('manualSearchInput').value = record.destination_name || '';
...
// --- Manual catalog search (NEXT_STEPS.md item 6) ---------------------------
//
// When none of the 3 auto-suggested candidates fit, this is what lets a
// reviewer search the whole catalog instead of only being able to click
// "Не подходит". Scoped to the SAME catalog this run is using
// (matchResult.catalog_version_id) - see standalone_matching.manual_search's
// own docstring for why that matters (the old Review tab's manual-search
// endpoint queries a different, catalog-agnostic global index).

async function runManualSearch() {
  const record = reviewQueue[wizardIndex];
  if (!record) return;
  const query = $('manualSearchInput').value.trim();
  const resultsEl = $('manualSearchResults');
  if (!query) {
    resultsEl.innerHTML = '';
    return;
  }
  if (!matchResult || !matchResult.catalog_version_id) {
    // Reusing an existing catalog always sets this; a genuinely fresh
    // upload does too, once ingested (see run_matching_job) - the only way
    // this is missing is a job result from before that field existed.
    resultsEl.innerHTML = '<p class="wizard-manual-search-label">Поиск недоступен для этого запуска.</p>';
    return;
  }
  ...
  try {
    const res = await fetch(`${API}/manual-search`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ query, catalog_version_id: matchResult.catalog_version_id, top_k: 10 }),
    });
    if (!res.ok) {
      resultsEl.innerHTML = '<p class="wizard-manual-search-label">Ошибка поиска.</p>';
      return;
    }
    const data = await res.json();
    renderManualSearchResults(record, Array.isArray(data.candidates) ? data.candidates : []);
  } catch (err) {
    resultsEl.innerHTML = '<p class="wizard-manual-search-label">Ошибка поиска.</p>';
  } finally {
...
function renderManualSearchResults(record, results) {
  const resultsEl = $('manualSearchResults');
  resultsEl.innerHTML = '';

  if (results.length === 0) {
    resultsEl.innerHTML = '<p class="wizard-manual-search-label">Ничего не найдено.</p>';
    return;
  }
  ...
  results.forEach((candidate) => {
    ...
    btn.addEventListener('click', () => {
      // Merge into record.candidates (if not already one of the original
      // 3) so every existing helper that looks a choice up there -
      // findCandidate/findCandidateName/findCandidateScore, the detail
      // views, buildSaveRows - keeps working unchanged on a manually-found
      // item exactly as it does on an auto-suggested one, with no special
      // casing needed anywhere else in this file.
      if (!findCandidate(record, candidate.catalog_id)) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('manualSearchInput').addEventListener('keydown', (event) => {
    if (event.key === 'Enter') {
      event.preventDefault(); // no surrounding <form> to submit, but stay explicit
      runManualSearch();
    }
  });
