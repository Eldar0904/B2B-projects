...
def test_attribute_rescoring_is_a_no_op_when_nothing_has_parsed_attributes(db_session):
    """The common case (attribute_score.py's own docstring): most real rows
    never get dim/material parsed at all. Phase 6.5 blending must not
    change the default fixture's ranking just because it now runs -
    compute_attribute_score returns the same NEUTRAL 0.5 for every
    candidate when nothing is comparable, which is a constant offset and
    ...
    from app.config import settings
    assert settings.attribute_score_weight > 0  # exercising the real default, not a stub

    candidates = matching.get_top_candidates(db_session, index, dest_products[0], top_k=3)
    assert candidates[0].master_product.id == master_products[0].id


def test_attribute_rescoring_can_promote_a_better_dimension_match(db_session):
    """Direct proof the Phase 6.5 wiring actually runs and does real work:
    construct a case where two master rows are near-identical textually
    (so retrieval/RRF alone can't strongly separate them), but only one
    shares the destination's real parsed dimensions - attribute blending
    should promote that one to rank 1, at the actual configured
    production weight (settings.attribute_score_weight, not a stub).
    """
    master_upload = Upload(filename="master.xlsx", upload_type="master", status="done")
    dest_upload = Upload(filename="dest.xlsx", upload_type="destination", status="done")
    db_session.add_all([master_upload, dest_upload])
    ...
    close_match = MasterProduct(
        upload_id=master_upload.id, source_row=2, external_id="M001",
        product_name="Шкаф для одежды", normalized_name="шкаф для одежды",
        unit="шт.", price=50000, is_group_header=False, raw_data={},
        dim_w_mm=800, dim_h_mm=1800, dim_d_mm=500, material="ЛДСП",
    )
    wrong_size = MasterProduct(
        upload_id=master_upload.id, source_row=3, external_id="M002",
        product_name="Шкаф для одежды", normalized_name="шкаф для одежды",
        unit="шт.", price=50000, is_group_header=False, raw_data={},
        dim_w_mm=300, dim_h_mm=400, dim_d_mm=200, material="ЛДСП",
    )
    dp = DestinationProduct(
        upload_id=dest_upload.id, source_row=2, external_id="D001",
        product_name="Шкаф для одежды", normalized_name="шкаф для одежды",
        quantity=1, price=50000, status="pending", raw_data={},
        dim_w_mm=800, dim_h_mm=1800, dim_d_mm=500, material="ЛДСП",
    )
    db_session.add_all([close_match, wrong_size, dp])
    db_session.commit()
    ...
    candidates = matching.get_top_candidates(db_session, index, dp, top_k=3)
    assert candidates[0].master_product.id == close_match.id
