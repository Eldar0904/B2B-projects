...
function guessColumnIndexes(headerRow) {
  ...
  return { name: findCol(PRICE_LIST_HEADER_HINTS.name), qty: findCol(PRICE_LIST_HEADER_HINTS.qty),
           price: findCol(PRICE_LIST_HEADER_HINTS.price), code: findCol(PRICE_LIST_HEADER_HINTS.code) };
}
...
function rowsFromSheet(sheetRows) {
  ...
  var dataStart = 1;
  if (cols.name === -1) { cols = { name: 0, qty: -1, price: -1, code: -1 }; dataStart = 0; }
...
function rowsToListText(rows) {
  return rows.map(function(r) { return r.qty != null ? r.name + ' x' + r.qty : r.name; }).join('\n');
}
